 ...
 !                  The difference between this program and Walk-1D-MSD-Test2-Pow-GPU is that
 !                  the jump distance is not fixed step size, but follows a pow-law distribution. 
 !                  Thus, this program can be used to distribution Levey flight.
 !                   
 !
 !
 ...
 program Walk_1D_MSD_Test2_Main
    ...
    character*18::SN1, SN2, SN3, SN4, SN5, SN6, SN7, SNS
    !--- Device variable
     character*32::ARGV
     integer::IDEV
     ...
     !---
       ...
       write(SN6,fmt="(I)") nint(DAV*100)
       write(SN7,fmt="(I)") nint(BETA*100)
       ...
       OutFname = "POW_TAU"//SN1(1:len_trim(SN1))//"_POW"//SN2(1:len_trim(SN2))//"_D0"//SN6(1:len_trim(SN6))//"_BEAT"//SN7(1:len_trim(SN7))&
                //"_NB"//SN5(1:len_trim(SN5))//"_TS"//SN4(1:len_trim(SN4))//"_TT"//SN3(1:len_trim(SN3))&
     ...
     !*** to initialize the DEVICE
       ...
       do while(CURT .le. TTIME)
          ...
          call Stepto_RecTime_Pow_1D_WithLJ_Test(1, NB, Tau, Pow, Dav, Beta, CURT, dPreTime, dNextTime, dNJump, dXP)
