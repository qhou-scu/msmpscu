...
sormain := $(MSMPSCUSOR)/MDLIB/sor/Analysis/$(objname)_Main.F90


#target directories
ifeq ($(ConfigName), Debug)
tgt  := $(WORKSPACE)/analytools/$(ConfigName)/
else
tgt  := $(WORKSPACE)/analytools/
endif
...
libs     += $(foreach n, $(boostmethslibnames), $(LIBDIRD)libMD_$(n).$(LIB_EXT))
libs     += $(foreach n, $(ltempctlibnames),    $(LIBDIRD)libMD_$(n).$(LIB_EXT))
...
liblist  += $(foreach n, $(boostmethslibnames), -L$(LIBDIRD) -lMD_$(n))
liblist  += $(foreach n, $(ltempctlibnames),    -L$(LIBDIRD) -lMD_$(n))
