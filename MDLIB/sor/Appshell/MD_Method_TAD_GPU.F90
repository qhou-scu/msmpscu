  ...
  implicit none
      ...
      private:: COMPARE_PRO
      abstract interface
        subroutine COMPARE_PRO(SimBoxIni, SimBox, CtrlParam, MASK, FLAG)
      ...
      procedure(COMPARE),     private, pointer::m_pCfgComp     => null()
 ...
 !--- END INTERFACE --------------------------------
  ...
  type(MDRecordStamp)::STAMP, STAMP0
            ...
            !*** prepare the control parameters for damping
              ...
              m_CtrlParamDamp%NB_UPTABMI    =  m_CtrlParamDamp%DAMPTIME1*2
              m_CtrlParamDamp%NB_UPTABMX    =  m_CtrlParamDamp%NB_UPTABMI
              m_CtrlParamDamp%NB_UPTAB      =  m_CtrlParamDamp%NB_UPTABMI
             ...
             !**** to determine how many inner-test loop we need
              ...
              if(CtrlParam%RESTART .EQ. 0 .or. .NOT.EXIST) then
                 ...
                 if(Simbox0%IniCfgID .eq. C_IZERO) then
                  ...
                  !--- from immediate point of the last simulation
                   ...
                   write(*,fmt="(A)")      ' MDPSCU Message: Load initial substrate from multi-file:'
              ...
              do ITEST =TESTLOOP0, TESTLOOP
                 ...
                 if(ITIME .EQ. 0 ) then
                   ...
                   call Do_DePhase(m_SimBoxIni(1), SimBox)
                ...
                if(iand(sectCtrlParam%AR_METHOD,CP_ENABLE_AR) .eq. CP_ENABLE_AR) then
                 ...
                 !*** trestore the original value of  MULTIBOX
  ...
  !----
                 ...
                 call STRCATI(GFILE0,  GFILE0, "_", JOB, 4)
                   ...
                   !   NOTE: in parallel-replica method no damping and thermalization anymore
                   !         after dephasing
                   ...
                   m_InterMXNum = (sectCtrlParam%TIMESTPG-1)/max(sectCtrlParam%PARREP_FineR,1) + 1
                   ...
                   do while(.TRUE.)
                      ...
                      call For_One_Step(ITIME, sectCtrlParam, SimBox)
                         ...
                         if(.not. HASCOPYOUT) then
                            ...
                            if(NCB .le.0) then
                               ...
                               if(m_NUMTgt .lt. EXITATEVENT ) then
                                  ...
                                  call Do_DePhase(m_SimBoxIni(1), SimBox(1:m_mxReplicas))
                                  ITime      = m_ITIMETgt
                                  Time       = m_TIMETgt
                                  TIME0      = Time
                                  ITIMEP     = ITime - sectCtrlParam%IT0 + 1
  ...
  real(KINDDF), save::DT, CURT0, CURT, VEL(3)
     ...
     !--- update the neighbore list
         if(MOD(ITIME,CtrlParam%NB_UPTAB) .eq. C_IZERO) then
            call Cal_NeighBoreList_DEV(SimBox, CtrlParam)
         end if

     !--- update the the active region
          if(mod(ITIME, CtrlParam%AR_UPTAB) .eq. C_IZERO) then
     ...
     !---  the modification when there is E-P coupling for ParRep MD
     !     we do not support E_P coupling
         !if(ITime .ge. CtrlParam%EPCTIME0 .and.   &
         !   ITime .le. CtrlParam%EPCTIME0 + CtrlParam%EPCTIME1-1) then
         ! call Do_EPCForce_DEV(SimBox(1), CtrlParam)
         !end if
  ...
  IMPLICIT NONE
   !---dummy vaiables
       ...
       integer,                    intent(in)::Reinit
           ...
           select case(CtrlParam%DAMPSCHEME)
                  case( CP_DAMPSCHEME_LBFGS)
                        call DO_DAMP_LBFGS(SimBox, CtrlParam, Reinit)
                  case(CP_DAMPSCHEME_DYN)
                        call DO_DAMP_DYN(SimBox, CtrlParam, Reinit)
            end select
  end subroutine DO_DAMP
  !****************************************************************************************

  !****************************************************************************************
  subroutine DO_DAMP_LBFGS(SimBox, CtrlParam, Reinit)
  !***  PORPOSE: to damping the configuration to zero temperature.
  !
  !    INPUT:  CtrlParam,  the control parameters for simulation
  !            REINIT,     indicating if re-initialization of device memory is needed
  ...
  IMPLICIT NONE
   !---dummy vaiables
      ...
      integer::I, MULTIBOX, MXITER, NB, IFLAG
           ...
           if(Reinit .gt. 0) then
             ...
             !*** to calculate the force
              ...
              !    It in not necessary to not calculate force here
              !    call  CalForce_ForceClass(SimBox(1), CtrlParam, gm_ForceClass)
           end if

           MXITER = CtrlParam%DAMPTIME1
     ...
     return
  end subroutine DO_DAMP_LBFGS
  !****************************************************************************************

  !****************************************************************************************
  subroutine DO_DAMP_DYN(SimBox, CtrlParam, Reinit)
  !***  PORPOSE: to damping the configuration to zero temperature.
  !
  !    INPUT:  CtrlParam,  the control parameters for simulation
  !            REINIT,     indicating if re-initialization of device memory is needed
  ...
  IMPLICIT NONE
   !---dummy vaiables
      ...
      integer::I, NB, MULTIBOX
           ...
           if(Reinit .gt. 0) then
             ...
             !*** if active region method to be used,  initialize the modeul
              ...
              !*** to calculate the force
              call CalForce_ForceClass(SimBox(1:NB), CtrlParam, gm_ForceClass)
           end if

           do I=1, CtrlParam%DAMPTIME1
              !--- Give a prediction
              call Predictor_DEV(I, SimBox(1), CtrlParam)

              !--- update the neighbore list
              if(mod(I,CtrlParam%NB_UPTAB) .eq. C_IZERO) then
                 call Cal_NeighBoreList_DEV(SimBox(1:NB), CtrlParam)
              end if

             !--- update the the active region
             if(mod(I, CtrlParam%AR_UPTAB) .eq. C_IZERO) then
                call ActivateRegion_DEV(SimBox(1:NB), CtrlParam)
             end if
              ...
              !--- to do the correction
              call Correction_DEV(I,SimBox(1), CtrlParam)
           end do
           ...
           call CalEkin_DEV(SimBox(1), CtrlParam)
           call CopyOut_SimBox_DEV(SimBox(1:NB))
           call Cal_thermal_quantities_SimBoxArray(SimBox(1:NB))
           write(*,fmt="(A, F12.3, A)")      ' MDPSCU Message: temp. after damping ', SimBox(1)%TEMPERATURE, ' K'
     ...
     return
  end subroutine DO_DAMP_DYN
  !****************************************************************************************

  !****************************************************************************************
  subroutine Do_DePhase(SimBoxIni, SimBox)
   ...
   !---dummy vaiables
      ...
      type(SimMDBox), dimension(:), allocatable::swapSimBox
           ...
           if(iand(m_CtrlParamDephase%AR_METHOD,CP_ENABLE_AR) .eq. CP_ENABLE_AR) then
               call Initialize_ActiveRegion_DEV(SimBox, m_CtrlParamDephase)
               call ActivateRegion_DEV(SimBox, m_CtrlParamDephase)
           end if

           !*** to calculate the force
           call CalForce_ForceClass(SimBox, m_CtrlParamDephase, gm_ForceClass)
          ...
          write(*,fmt="(A, I7, A)")  ' MDPSCU Message: to thermalize the replicas'
              ...
              do ITIME=1, (m_CtrlParamDephase%IVTIME+1)*m_CtrlParamDephase%IVPAS
                 if(MOD(ITIME - m_CtrlParamDephase%IVTIME0+1, m_CtrlParamDephase%IVPAS) .EQ. C_IZERO) then
                    call CalEpot_ForceClass(SimBox(1:m_mxReplicas),  m_CtrlParamDephase, gm_ForceClass)
                ...
                if(mod(ITIME, m_CtrlParamDephase%AR_UPTAB) .eq. C_IZERO) then
                 ...
                 !--- calculate the current force
                 call CalForce_ForceClass(SimBox(1:m_mxReplicas), m_CtrlParamDephase, gm_ForceClass)
              ...
              call Do_Damp(SwapSimBox(1:m_mxReplicas), m_CtrlParamDamp, REINIT=0)
  ...
  IMPLICIT NONE
   !---dummy vaiables
      ...
      type(SimMDBox), dimension(:), allocatable::SwapBox
           ...
           call Do_Damp(SwapBox, m_CtrlParamDamp, REINIT=REINITGPU)
           !---
           Mask = 1
           if(associated(m_pCfgCompMask) ) then
