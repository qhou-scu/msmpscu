  module MD_FS_Force_Table_GPU
  !**** DESCRIPTION: _______________________________________________________________________________________
  !                  To calculate the forces on atoms USING GPU paralle comuputing, the force is assumed in
  !                  tight-binding of Finnis-Sinclar: Ec=-DQRT(RO)+SUMij(Vij) with RO the total electron density
  !                  on atoms; Vij,the pairwise portential on atom i by atom j.
  !
  ...
  !
  !                  ______________________________________________________________________________________
  !**** HISTORY:     The version is addapted from  MD_FS_Force_Table_GPU, a version supporting
  !                  only one device. In the present version, multiple (six) devices can be used.
  !                  2012-04 (Hou Qing)
  !
  ...
  !                               force calculations.
  !     
  !                  2019-01-02,  copyout of DEN on device that was performed in PRECALFOR_template are 
  ...
  !-----------------------------------------------
    implicit none
      ...
      !$$real(KINDDF), constant, private::dcm_RU2(mp_MXGROUP,mp_MXGROUP)
  ...
  !*********************************************************************************
    subroutine Allocate_ForceTable_template (IDEV, FTable, POTR,FPOTR, POTB, FPOTB, DEN, VTENSOR)
    ...
    implicit none
      ...
      integer::ERR, CURDEV, BX, BY, NBX, NBY, NB
           ...
           !$$
               ...
               if(ERR) then
                  write(*,*) "MDPSCU Error: fail to allocate memory for DEN in MD_FS_ForceTable on device", IDEV
                  write(*,*) "              number of atoms is ", dm_NPRT
                  stop "1"
               end if
               DEN      = C_ZERO
               VTENSOR  = C_ZERO
               !$$--- copyin the constant memory
               dcm_KPAIR= m_KPAIR
               dcm_IFPD = m_IFPD

               allocate(POTR(m_TABDIM(1), m_TABDIM(2)), POTB(m_TABDIM(1), m_TABDIM(2)),              &
                        FPOTR(m_TABDIM(1),m_TABDIM(2)), FPOTB(m_TABDIM(1),m_TABDIM(2)),STAT=ERR  )

               if(ERR) then
                  write(*,*) "MDPSCU Error: fail to allocate device memory for force table in MD_FS_ForceTable", IDEV
                  stop "2"
               end if
  ...
  !*********************************************************************************
    subroutine INITIALIZE_FS_Force_Table_DEV(SimBox, CtrlParam, FTable,RelaseTable, MULTIBOX)
    ...
    !
     ...
     implicit none
      ...
      logical::pinnedFlag
           ...
           return
    end subroutine INITIALIZE_FS_Force_Table_DEV
    !*********************************************************************************

    !*********************************************************************************
    subroutine Clear_Force_Table_template(IDEV, POTR,FPOTR, POTB, FPOTB, DEN,VTENSOR)
  ...
  100    write(*,*) "MDPSCU Error in deallocate device memory in FINNIS_EM_TB_Force_Table"
   ...
   !*********************************************************************************
    subroutine Clear_FS_Force_Table_DEV()
  ...
  100    write(*,*) "MDPSCU Error in deallocate device memory in FINNIS_EM_TB_Force_Table"
         return
    end subroutine CLEAR_FS_Force_Table_DEV
  !*********************************************************************************

  !*********************************************************************************
   attributes(global) subroutine PRECALFOR_KERNEL(IM, ITYP,XP, NAPDEV, STATU, IA0, NPRT, KVOIS,INDI, CSI, RU2, KT, POTB, DEN,   &
  ...
  !
  !$$   OUTPUT:    DEN,   1/sqrt(eelctron density)
  !
              ...
              if(IC.LE.NPRT) then
                 ...
                 if(iand(STATU(IC), CP_STATU_ACTIVE) .eq. CP_STATU_ACTIVE ) then
                   ...
                   !$$-- start calculation of electron density
                    ...
                    !$$**** NOTE: need to change the following if EAM potential is used
                    if(DEN0 .GT. C_ZERO) then
                      !$$--- NOTE by Hou Qing, Dec 4,2014
                      !$$       in old version  the force term FPOTB has been multiplied by -0.5
                      !$$       in new version, we do not multiply 0.5 on FPOTB
                      !$$       thus we should chnage
                      !$$         DEN0 = -C_UN/DSQRT(m_DEN(I))
                      !$$       to
                      !$$         DEN0 = C_HLAF/DSQRT(m_DEN(I))
                      !DEN0 = -C_UN/DSQRT(DEN0)
                       DEN0 = -C_HALF/DSQRT(DEN0)
                    end if
                 end if
  ...
  !*********************************************************************************
  subroutine PRECALFOR_template(IDEV, STARTCELL, ENDCELL, KVOIS, INDI, ITYP, XP, STATU, POTB, DEN)
  ...
  !
   ...
   implicit none
      ...
      !--- Local variables
         integer::BX, BY, NB, NBX, NBY, ERR, STARTA, ENDA, NPRT, IA0
      ...
      !$$--- Device variables and variables to be used in GPU
        ...
        !$$    NB  = NBX*NBY
             ...
             call PRECALFOR_KERNEL<<<blocks, threads>>>(dm_NPRT,ITYP,XP, m_NAPDEV, STATU, IA0, NPRT, KVOIS,INDI, &
                                                        m_CSI, m_MXRU2, m_TABDIM(1), POTB, DEN,                  &
  ...
  !
   ...
   implicit none
      ...
      !$$--- Device variables and variables to be used in GPU
        ...
        !$$-- we need to copyin DEN, because the subscript of
        !$$   DEN in the index of the whole box, just like XP
        !$$   we need copy the DEN on device into host
        !$$    DEN = hm_DEN
        !$$    ERR     = cudaMemcpyAsync(DEN, hm_DEN,dm_NPRT)
        !$$   This operatione is performed by call Synchroniz_DEN_on_Devices(), /2019-01-02 HOU Qing   
  ...
  !*********************************************************************************
  subroutine CALDEN_FS_Force_Table2A_DEV(SimBox, CtrlParam)
  ...
  !
  !     SEE ALSO:  CALFORCE_FS_Force_Table2A_DEV
  !                CALFTENSOR_FS_Force_Table2A_DEV
  !                Cal_NeighBoreList_DEV
  !
  !
   use MD_Globle_Variables_GPU
        ...
        !$$--- caluclate the -dFi(RHO)/d(RHO) on atoms
             do I=1, m_NDEVICE
                 call PRECALFOR_template(m_DEVICES(I), m_STARTCELL(I), m_ENDCELL(I),                                      &
                                         ...
                                         dm_FtableWS(I)%POTB,        dm_FtableWS(I)%DEN)
             end do
          ...
          return
  end subroutine CALDEN_FS_Force_Table2A_DEV
  !*********************************************************************************

  !*********************************************************************************
  subroutine CALFORCE_FS_Force_Table2A_DEV(SimBox, CtrlParam)
  !***  PURPOSE:   to calculate the forces. For speedup, no virial tensor
  !                are calculted (ref;CALPTENSOR_FS_Force_Table2A_DEV)
  !
  !     INPUT:     SimBox,    the simulation box
  !                CtrlParam: the control parameters
  !
  ...
  !
  !     SEE ALSO:  CALFTENSOR_FS_Force_Table2A_DEV
   ...
   implicit none
      ...
      !--- Local variables
        ...
        !$$--- caluclate the forces on atoms
          ...
          return
  end subroutine CALFORCE_FS_Force_Table2A_DEV
  !*********************************************************************************
                 ...
                 if(iand(STATU(IC), CP_STATU_ACTIVE) .eq. CP_STATU_ACTIVE ) then
                    ...
                    do IW=1, IIW
                       ...
                       if(R2 .LE. RU2) then
                          ...
                          !FORTOT= EXP5 + EXP7*DENKI + EXP6*DENKJ
  ...
  !*********************************************************************************
  subroutine CALPTENSOR_FS_Force_Table2A_DEV(SimBox, CtrlParam)
   ...
   implicit none
      ...
      !--- Local variables
        ...
        !$$--- caluclate the -dFi(RHO)/d(RHO) on atoms
          ...
          return
  end subroutine CALPTENSOR_FS_Force_Table2A_DEV
  !*********************************************************************************
  ...
  !*********************************************************************************
   attributes(global) subroutine CALEPOT_KERNEL(IM, ITYP,XP, NAPDEV, STATU, IA0, NPRT, KVOIS,INDI, CSI, RU2, KT, POTR, POTB, EPOT,&
                                                BX0, BY0, BZ0, BS011, BS012, BS013, BS021, BS022, BS023, BS031, BS032, BS033)
  ...
  !
      ...
      integer::IC, IT, IB !, LL, NAL,NL
              ...
              if(IC.LE.NPRT) then
                 ...
                 !$$**** NOTE: 1. need to modify the following if EAM potential is used
                 !$$           2. the subscript of EPOT is the index of atoms on the device
  ...
  !*********************************************************************************
  subroutine CALEPOT_template(IDEV, STARTCELL, ENDCELL, KVOIS, INDI, ITYP, XP, STATU, POTR, POTB, dEPOT)
      ...
      !--- Local variables
        ...
        !$$-- to determine the dimension of blocks( the number of blocks in a grid)
            ...
            call CALEPOT_KERNEL<<<blocks, threads>>>(dm_NPRT, ITYP, XP, m_NAPDEV, STATU, STARTA, NPRT, KVOIS,INDI,  &
                                                      m_CSI, m_MXRU2, m_TABDIM(1), POTR, POTB, dEPOT,               &
                                                      m_BOXSIZE(1),    m_BOXSIZE(2),    m_BOXSIZE(3),               &
                                                      m_BOXSHAPE(1,1), m_BOXSHAPE(1,2), m_BOXSHAPE(1,3),            &
                                                      m_BOXSHAPE(2,1), m_BOXSHAPE(2,2), m_BOXSHAPE(2,3),            &
                                                      m_BOXSHAPE(3,1), m_BOXSHAPE(3,2), m_BOXSHAPE(3,3)   )
  ...
  !*********************************************************************************
  subroutine UpdateEPOT_FS_Force_Table2A_DEV(SimBox, CtrlParam)
  !***  PURPOSE:   to calculate the potential for each atom, 
  !                and copy the data into dm_EPOT
  !
  !
  !     INPUT:     SimBox,    the simulation box
  !                CtrlParam: the control parameters
        ...
        !$$--- caluclate the EPOT of atoms on devices
             do I=1, m_NDEVICE
                call CALEPOT_template(m_DEVICES(I), m_STARTCELL(I), m_ENDCELL(I),                                            &
                                         dm_Neighbors%KVOIS(I)%Data, dm_Neighbors%INDI(I)%Data,                              &
                                         dm_WorkSpace%ITYP(I)%Data,  dm_WorkSpace%XP(I)%Data,    dm_WorkSpace%STATU(I)%Data, &
                                         dm_FTableWS(I)%POTR,        dm_FTableWS(I)%POTB,                                    &
                                         dm_WorkSpace%EPOT(I)%Data)
             end do

             ERR = cudaSetDevice(CURDEV)
          return
  end subroutine UpdateEPOT_FS_Force_Table2A_DEV
  !*********************************************************************************

  !*********************************************************************************
  subroutine CALEPOT_FS_Force_Table2A_DEV(SimBox, CtrlParam)
  ...
  !
   ...
   implicit none
      ...
      !--- Local variables
            ...
            call UpdateEPOT_FS_Force_Table2A_DEV(SimBox, CtrlParam)
            call CopyEPOTFrom_Devices_to_Host()

          return
  end subroutine CALEPOT_FS_Force_Table2A_DEV
  !*********************************************************************************

  !*********************************************************************************
   attributes(global) subroutine CAL_FS_AtomicStress_KERNEL(IM, ITYP, XP, NAPDEV, STATU, IA0, NPRT, KVOIS, INDI, CSI, RU2, KT, &
                                                            FPOTR, FPOTB, DEN, AP,                                             &
                                                            BX0, BY0, BZ0, BS011, BS012, BS013, BS021, BS022, BS023, BS031, BS032, BS033)
  ...
  !
      ...
      integer::IC, IT, IB, LL, NL, NAL
        ...
        RETURN
  END subroutine CAL_FS_AtomicStress_KERNEL
  !*************************************************************************************

  !*********************************************************************************
  subroutine CAL_FS_AtomicStress_template(IDEV, STARTCELL, ENDCELL, KVOIS, INDI, ITYP, XP, STATU, FPOTR, FPOTB, DEN, AVP)
   ...
   implicit none
      ...
      !--- Device variables and variables to be used in GPU
         ...
         type(dim3) :: threads
            ...
            threads = dim3(BX, BY, 1)
            STARTA  = STARTA-1
            call CAL_FS_AtomicStress_KERNEL<<<blocks, threads>>>(dm_NPRT, ITYP, XP, m_NAPDEV, STATU, STARTA, NPRT, KVOIS,INDI,  &
                                                        ...
                                                        m_BOXSHAPE(3,1), m_BOXSHAPE(3,2), m_BOXSHAPE(3,3)   )
          return
  end subroutine CAL_FS_AtomicStress_template
  !*********************************************************************************

  !*********************************************************************************
  subroutine Cal_FS_AtomicStressTensor_DEV(IDEV, dAVP)
  ...
  !     SEE ALSO:  CALDEN_FS_Force_Table2A_DEV
  !                CALFORCE_FS_Force_Table2A_DEV
  !
  !
   use MD_Globle_Variables_GPU
   ...
   implicit none
      ...
      !--- Local variables
         ...
         !$$
          call CAL_FS_AtomicStress_template(m_DEVICES(IDEV), m_STARTCELL(IDEV), m_ENDCELL(IDEV),                                         &
                                             ...
                                             dAVP)
          return
  end subroutine Cal_FS_AtomicStressTensor_DEV
  !*********************************************************************************
  ...
  end module MD_FS_Force_Table_GPU
