 ...
 !                  The program is used to calculate the jump rate of interstial on HCP lattice.
 ...
 implicit none
  ...
  implicit none
       ...
       !--- local variables
        ...
        integer, dimension(:),allocatable::hSTAT, hOCCUa, hSITEi, hOCCUai
         ...
         !--- calculate the occupation of lattice sites
          ...
          !--- to extract the postion of interstitial
           ...
           if(m_NREC .eq. 0 ) then
              ...
              m_curFTimei = 0.D0
              m_NREC = m_NREC + 1
              !$$--- check if there is atom out of box
              m_BOXFLAG  = 0
              do IB=1, NBOX
              ...
              deallocate(hSitePOS, hSTAT, hOCCUa, hSITEi, hOCCUai)
              return
           end if
             ...
             !--- to caluclate the jumping time and displacement
              do I=1, NOLD
                ...
                !--- up date the
                 ...
                 write(m_j_v_IOUnit1, fmt="(1x, I8, 1x, I8, 16(1x, 1PE13.4,1x))") (JOB-1)*NBOX, EVENTTYP(I), DTIME(I), COSDIR, &
  ...
  !
   implicit none
       ...
       type(MDRecordStamp)::STAMP
              ...
              write(*, fmt="(A)") "Move change events historty to be saved in "//m_OUTPATH(1:len_trim(m_OUTPATH))//"Interstial.Time_Displace"
              write(*, fmt="(A)") "Events historty to be saved in "//m_OUTPATH(1:len_trim(m_OUTPATH))//"Interstial.Time_Displace1"
              close(m_j_v_IOUnit)
              close(m_j_v_IOUnit1)

            !write(*, fmt="(A)") "Jumping vector to be saved in "//m_OUTPATH(1:len_trim(m_OUTPATH))//"Inter_Jump_Vector"
            !call Putout_Instance_Config_SimMDBox(0, 1, 0, 0.D0, m_OUTPATH(1:len_trim(m_OUTPATH))//"Inter_Jump_Vector", swapSimBox)
