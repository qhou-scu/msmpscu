 ...
 100          write(*,*) "MDPSCU ERROR: Fail to allocate memory in VORONOI_TESSLATION MODULE on device", IDEV
    ...
    implicit none
      ...
      !--- Local variables
           ...
           !$$--- print out the control parameters
            write(hFile,fmt="(A)") " !************ VoronoiTessellationM_GPU module to be performed **********"
            ...
            do LL=1, NL
              ...
              if(IC.LE.NPRT) then
                 ...
                 if(AMASK(ICS) .NE. mp_AMASK_SEL) then
                  ...
                  if(VERT(IVERT0+1) .le. 0 .or. &
                     VERT(IVERT0+2) .le. 0 .or. &
                     VERT(IVERT0+3) .le. 0 ) then
                     STAT(ICS) =  mp_VSTAT_OPENED
                     cycle
                  end if 
                        ...
                        do IW1=1, MXNV !--- change IIW  to MXNV, because the permitte number of vertices is MXNV
                           ...
                           do IW=1, min(IIW, mp_MXKVOIS) !--- the number of candidate vertices is the number of neighbore
                              ...
                              !$$    of current moving face
                               if(abs(DET2) .lt. underflow) cycle
                           ...
                           !$$--- if all neighbore have been exhausted without find new vertice, exit
                           if(I4 .LT. 0 ) then
                              STATU  = mp_VSTAT_OPENED   
                              exit  !
                           end if   
  ...
  !
   ...
   !--- Device variables and variables to be used in GPU
        ...
        type(dim3) :: threads
             ...
             ERR = cudaSetDevice(IDEV)
             ERR   = cudaMemsetAsync(DVTS, 0, size(DVTS), 0)
             ...
             IA0     = IA0 - 1
             call Cal_Delaunay_Vertice_KERNEL0<<<blocks, threads>>>(dm_NPRT,XP, AMASK, HMASK, m_NAPDEV, IA0, IAS, NPRT, m_MXNF, &
  ...
  !
   ...
   !---Device variables and variables to be used in GPU
       ...
       type(dim3) :: threads
             ...
             threads = dim3(BX, BY, 1)
             IA0     = IA0-1 
             call Cal_Delaunay_Vertice_KERNEL1<<<blocks, threads>>>(dm_NPRT,XP, AMASK, HMASK, m_NAPDEV, IA0, IAS, NPRT, m_MXNF, m_MXNV,&
  ...
  !
   ...
   !---Local variables
       integer::BX, BY, NB, NBX, NBY, ERR, IA0, IAS, ENDA, NPRT, SVV
   ...
   !---Device variables and variables to be used in GPU
       ...
       type(dim3) :: threads
             ...
             ERR = cudaSetDevice(IDEV)
             ERR   = cudaMemsetAsync(VVERT, 1.E32, size(VVERT), 0)
             ...
             threads = dim3(BX, BY, 1)
             IA0 = IA0 -1 
             call Cal_Voronoi_KERNEL<<<blocks, threads>>>(dm_NPRT,XP, m_NAPDEV, IA0, IAS, NPRT, m_MXNF, &
 ...
 !**********************************************************************************
  ...
  !-------
           ...
           call Reorder_NeighBoreList_Nearest_DEV(2*m_MXNV )
