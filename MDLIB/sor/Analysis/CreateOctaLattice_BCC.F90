 ...
 implicit none
  ...
  !**********************************************************************************
   subroutine Initialize_OctaSite(SimBox, CtrlParam)
  ...
  !----   Local variables
   ...
   real(KINDDF)::RMAT(3,3)
         ...
         return
   end subroutine Initialize_OctaSite
  !**********************************************************************************

  !**********************************************************************************
  subroutine CreateOctaSite(SimBox, CtrlParam, TN, TXP)
...
!-----  Local varaible
        ...
        real(KINDDF)::RR, DIST, SEP(3), XPP(3), BL(3), BU(3), BS(3), HBS(3), PTX(mp_NPT), PTY(mp_NPT), PTZ(mp_NPT), RC(mp_NPT)
            ...
            NPRT = SimBox%NPRT
            allocate(IND(NPRT, mp_NPT), XP(NPRT,3), ISP(NPRT*mp_NPT2))
            ...
            !$$--- start construct the octahdral sites
            TN0 = 0
            do I=1, NPRT
               ...
               do K=1, mp_NPT2
                  ...
                  J2  = mp_PIAR(2*K)
                  TN0 = TN0 + 1
                  TXP(TN0, 1) = 0.5D0*PTX(J1) + 0.5D0*PTX(J2) + XP(I,1)
                  TXP(TN0, 2) = 0.5D0*PTY(J1) + 0.5D0*PTY(J2) + XP(I,2)
                  TXP(TN0, 3) = 0.5D0*PTZ(J1) + 0.5D0*PTZ(J2) + XP(I,3)
               end do
            end do
            ...
            !$$--- to move the sites out side the box into box
            TN = 0
            do I=1, TN0
               if(ISP(I) .gt. 0) then
          ...
          return
  end subroutine CreateOctaSite
  !**********************************************************************************

  !**********************************************************************************
  subroutine CreateOctaSiteBox(SimBox, CtrlParam, OctaBox)
  !***  DESCRIPTION: to create the TetrahdralSites box
  !
  !    INPUT:  SimBox,      the simulation boxs
  !            CtrlParam,   the control parameters for simulation
  !
  !    OUTPUT: OctaBox,     the simulation box with atom position at the octahedral sites
  !                         of box SimBox
  !
  use MD_NeighborsList
  implicit none
       type(SimMDBox),  intent(in)::SimBox
       type(SimMDCtrl), intent(in)::CtrlParam
       type(SimMDBox)             ::OctaBox

!-----  Local varaible
       integer::TN
        real(KINDDF), dimension(:,:), allocatable::TXP

            allocate(TXP(SimBox%NPRT*mp_NPT2,3))
            call CreateOctaSite(SimBox, CtrlParam, TN, TXP)
            call AddAtoms_SimMDBox(OctaBox, TN, ITYPE=SimBox%NGROUP+1, RXP=TXP, TYPEORDER=0)
            deallocate(TXP)
            return
  end subroutine CreateOctaSiteBox
  !**********************************************************************************

  !**********************************************************************************
  subroutine Record_OctaSite(Stamp, SimBox, CtrlParam)
...
!-----  Local varaible
        ...
        type(MDRecordStamp)::SWAPSTAMP
            ...
            call CreateOctaSiteBox(SimBox(1), CtrlParam, SWAPBOX)
            SWAPSTAMP%ITime = 0
            SWAPSTAMP%ITEST = 1
            SWAPSTAMP%ISect = 1
            SWAPSTAMP%ICfg  = 0
            SWAPSTAMP%IRec  = 0
            SWAPSTAMP%Time  = 0
            write(*,fmt="(A, I6, A)") " Octahedral latiice to be output to "//&
          ...
          return
  end subroutine Record_OctaSite
  !**********************************************************************************
 end module CreateOctaLattice_BCC
