 ...
 module DefectCluster_Analysis_hcp
 use LatticeClusterTrackCommon
 implicit none
        real(KINDDF), parameter, private::m_VECT1(3) = (/1.D0,   0.D0,        0.0D0/)
        real(KINDDF), parameter, private::m_VECT2(3) = (/0.5D0,  0.8660254D0, 0.0D0/)
        real(KINDDF), parameter, private::m_VECT3(3) = (/-0.5D0, 0.8660254D0, 0.0D0/)
        real(KINDDF), parameter, private::m_EPSLON = 1.D-6
        integer, parameter::mp_VSIZ = 12

 contains

 !**********************************************************************************
  subroutine DefectBondInfo0_HCP(Cluster, BONDS, FLAG)
  ...
  implicit none
       ...
       !--- local variables
        ...
        real(KINDDF)::RR, DIST, XP0(3), XPP(3), BS(3), HBS(3)
             ...
             FLAG = 0
                ...
                do I=1, Cluster%NPRT
                   ...
                   do J=I+1, Cluster%NPRT
                      ...
                      if(dabs(XPP(3)) .gt. m_EPSLON) then    !--- for off basal sites
                         SH2 = 4
                      else
                         DIST = dsqrt(XPP(1)*XPP(1) + XPP(2)*XPP(2))
                         if(dabs(XPP(1)*m_VECT1(1)+XPP(2)*m_VECT1(2))/DIST .gt. 0.99) then
                            SH2 = 1
                         else if(dabs(XPP(1)*m_VECT2(1)+XPP(2)*m_VECT2(2))/DIST .gt. 0.99) then
                            SH2 = 2
                         else  if(dabs(XPP(1)*m_VECT3(1)+XPP(2)*m_VECT3(2))/DIST .gt. 0.99) then
                            SH2 = 3
                         else
                            !FLAG = FLAG + 1
                         end if
             ...
             return
  end subroutine DefectBondInfo0_HCP
  !**********************************************************************************

 !**********************************************************************************
  subroutine DefectBondInfo1_HCP(Cluster, BONDS, FLAG)
  !***  DESCRIPTION: to find out shape of the defect cluster. The shape is defined as number
  !                  bonds in directions. 12 intergers are used to describe the shape.
  !                  The first four integers are the number of bonds between LSIAs. The first
  !                  three integers are number of bonds in [2110] drections, the fouth integer
  !                  is the number of off-plane bonds.
  !                  The difference between this and  DefectBondInfo0_HCP is that the output
  ...
  implicit none
       ...
       !--- local variables
        integer::I, J, TSP(mp_VSIZ), ISWAP(4)
                ...
                call DefectBondInfo0_HCP(Cluster, TSP, FLAG)
             ...
             return
  end subroutine DefectBondInfo1_HCP
  !**********************************************************************************

  !**********************************************************************************
  subroutine DefectBondInfo_HCP(Clusters, SNUM, VNUM, ANUM, BONDS, FLAG)
  !***  DESCRIPTION: to find out shape of the defect cluster. The shape is defined as number
  !                  bonds in directions. 12 intergers are used to describe the shape.
  !                  The first four integers are the number of bonds between LSIAs. The first
        ...
        type(PartcleClusterList), pointer::curC
             ...
             do while(associated(curC))
                ...
                IC  = IC + 1
                 ...
                 call DefectBondInfo1_HCP(curC, BONDS((IC-1)*mp_VSIZ+1:IC*mp_VSIZ), FLAG0)
                 FLAG = FLAG + FLAG0
                !--- move to next cluster
                curC => curC%next
             end do
             return
  end subroutine DefectBondInfo_HCP
 !**********************************************************************************

  !**********************************************************************************
  subroutine DefectJumpAnalysis(Cluster1, Cluster2, NE, EVENT, DEFTYP, DIR, NNS, NNV, STAT)
  !***  DESCRIPTION: to analyse jumping event from cluster 1 to cluster2
  !                  for hcp.
  !
  !
...
!print *, "here1", I, EVENT(I), Cluster1%PTYP(IS1(I)), Cluster1%PTYP(IS2(I)), DEFTYP(I)
...
!print *, "here2", I, EVENT(I), Cluster1%PTYP(IS1(I)), Cluster1%PTYP(IS2(I)), DEFTYP(I)
...
!print *, "here3", I, IS1(I), IS2(I), Cluster1%IND(IS1(I)), Cluster1%IND(IS2(I)), IT
...
!print *, "here4", I, J, ID1, Cluster1%PTYP(IT), Cluster1%IND(J), Cluster1%PTYP(J), any(m_SiteNeighbor%INDI(ID1,1:NN) .eq. Cluster1%IND(J)), NNS(I)
                   end do
                   do J=1,  Cluster1%NPRT
 ...
 end module DefectCluster_Analysis_hcp
