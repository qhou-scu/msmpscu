  ...
  implicit none
     ...
     private::Load_ARTCtl_SimMDCtrl, &
              Load_SectionParameter
  contains
  !*********************************************************************

  !****************************************************************************
  subroutine Load_ARTCtl_SimMDCtrl(hFile, CtrlParam, STR, LINE)
              ...
              select case (KEYWORD(1:LEN_TRIM(KEYWORD)) )
                     ...
                     case("&TERMINATE")
                          ...
                          if( CtrlParam%ITE.lt.0) then
                              write(*,fmt="(' MDPSCU Error: number of time step smaller than 0')")
                              write(*,fmt="('               check control file at line:', BZI6)") LINE
                              write(*,fmt="(' Usage:  &TERMINATE  max number of timestep= mxsteps')")
                     ...
                     case("&STEPSIZE")
                          ...
                          if(N .LT. 1) then
                             write(*,fmt="(' MDPSCU Error: time step size (in ps)is expected')")
                             write(*,fmt="('               check control file at line:', BZI6)") LINE
                             write(*,fmt="(' Usage:  &STEPSIZE  h')")
                             write(*,fmt="(' Process to be stopped')")
                             stop
                          end if
                          CtrlParam%HMI = DRSTR(STRNUMB(1))
                          CtrlParam%HMX = DRSTR(STRNUMB(1))
                   ...
                   case ("&EXTRECORD")
                     ...
                     case ("&ACTPERCENT")
                          !$$*** To get percentage of atoms to be activated
                           call Extract_Numb(STR,1, N,STRNUMB)
                           CtrlParam%ART_ActPecent = DRSTR(STRNUMB(1))

                     case ("&ALPHA")
                          !$$*** To get alpha parameters
                           call Extract_Numb(STR,1, N,STRNUMB)
                           if(N .ge. 1) then
                     ...
                     case ("&DISPSTEP")
                          !$$*** To get step lenght for activating atoms
                           call Extract_Numb(STR,1, N,STRNUMB)
                           if(N .ge. 1) then
  ...
  100    write(*,*)"MDPSCU Error in reading time step control parameters."
         ...
         stop
 end subroutine Load_ARTCtl_SimMDCtrl
 !****************************************************************************
             ...
             do while(.TRUE.)
                ...
                select case (KEYWORD(1:LEN_TRIM(KEYWORD)) )
                     ...
                     case default
                       ...
                       case ("&ARTSUBCTL")
                             call Load_ARTCtl_SimMDCtrl(hFile, CtrlParam, STR, LINE)
