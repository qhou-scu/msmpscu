  module MD_SimMDCtrl_TAD
  !***  DESCRIPTION: this module is to load control parameters for TAD
  !
  ...
  implicit none
     ...
     private::Load_TemperaturCtl_SimMDCtrl,      &
              Load_TimestepCtl_SimMDCtrl,        &
              Load_SectionParameter_SimMDCtrl,   &
              Load_PARREPCtl_SimMDCtrl,          &
              Load_TADCtl_SimMDCtrl
  contains
  !*********************************************************************

  !****************************************************************************
  subroutine Load_TemperaturCtl_SimMDCtrl(hFile, CtrlParam, STR, LINE)
                  ...
                  select case (KEYWORD(1:LEN_TRIM(KEYWORD)) )
                          ...
                          case("&THERMALIZATION")
                               !$$*** To get how many times of thermalization will be carried out
                                 ...
                                 if(n .ge. 1) then
                                    ...
                                    if(STRTMP(1)(1:len_trim(STRTMP(1))) .eq. "MC") then
                                       CtrlParam%IVSCHEME = CP_THERMALSCHEME_MC
                                    else if(STRTMP(1)(1:len_trim(STRTMP(1))) .eq. "VSCAL") then
                                       CtrlParam%IVSCHEME = CP_THERMALSCHEME_VSCAL
                                    else if(STRTMP(1)(1:len_trim(STRTMP(1))) .eq. "PSCAL") then
                                       CtrlParam%IVSCHEME = CP_THERMALSCHEME_PSCAL
                                    else if(STRTMP(1)(1:len_trim(STRTMP(1))) .eq. "ASCAL") then
                                       CtrlParam%IVSCHEME = CP_THERMALSCHEME_ASCAL
                                    else
                                        write(*,fmt="(A)")      " MDPSCU Error: the thermalizing scheme "//STRTMP(1)(1:len_trim(STRTMP(1)))//" is unknown"
                         ...
                         case("&E_P_COUPLE")
                                !$$*** To get if electron-phono coupling is required
                                 write(*,fmt="(' MDPSCU Warning: for TAD MD, E-P coupling is not supported')") LINE
                                 write(*,fmt="(A, BZI6)")'               check control file at line:', LINE
                                 call ONWARNING(gm_OnWarning)
 ...
 !----------------------------------------------------------------------------
  100    write(*,*)"MDPSCU Error in reading temperature control parameters."
         ...
         stop
  end subroutine Load_TemperaturCtl_SimMDCtrl
  !****************************************************************************

  !****************************************************************************
  subroutine Load_TimestepCtl_SimMDCtrl(hFile, CtrlParam, STR, LINE)
  !***  PURPOSE:   to load the control parameter for temperature control
  !
  !     INPUT:     hFile,  I/O unit number
  !                STR,    a string line
  ...
  100    write(*,*)"MDPSCU Error in reading time step control parameters."
         ...
         write(*,*)"Please check if keyword &ENDSUBCTL exist to end the data subsection"
         stop
 end subroutine Load_TimestepCtl_SimMDCtrl
 !****************************************************************************

 !****************************************************************************
  subroutine Load_SectionParameter_SimMDCtrl(hFile, CtrlParam, SimBox, STR, LINE, ISECT)
             ...
             do while(.TRUE.)
                ...
                select case (KEYWORD(1:LEN_TRIM(KEYWORD)) )
                     ...
                     case default
                       ...
                       case ("&TEMPSUBCTL")
                            !*** start the temprature controlling parametgers
                            call Load_TemperaturCtl_SimMDCtrl(hFile, CtrlParam, STR, LINE)
                       ...
                       case ("&TIMESUBCTL")
                             call Load_TimestepCtl_SimMDCtrl(hFile, CtrlParam, STR, LINE)
                       ...
                       case ("PARREPSUBCTL")
                              call Load_PARREPCtl_SimMDCtrl(hFile, CtrlParam, STR, LINE)
              ...
              if(CtrlParam%TimestpR .le. 0) CtrlParam%TimestpR = max(CtrlParam%TimestpG, 100)/100
  ...
  100    write(*,*)"MDPSCU Error in reading section parameters."
         ...
         stop
  end subroutine Load_SectionParameter_SimMDCtrl
 !****************************************************************************

 !****************************************************************************
  subroutine Load_PARREPCtl_SimMDCtrl(hFile, CtrlParam, STR, LINE)
     ...
     !----
           ...
           do while(.TRUE.)
              ...
              select case (KEYWORD(1:LEN_TRIM(KEYWORD)) )
                     ...
                     case ("&FINESTEPS")
                          !$$--- fine time steps of svaing intermediate configuration
                          call Extract_Numb(STR,2,n,STRNUMB)
                          if(N .LT. 1) then
  ...
  100    write(*,*)"MDPSCU Error in reading parallel-replica control parameters."
         ...
         write(*,*)"Please check if keyword &ENDSUBCTL exist to end the data subsection"
         stop
 end subroutine Load_PARREPCtl_SimMDCtrl
 !****************************************************************************

  !****************************************************************************
  subroutine Load_TADCtl_SimMDCtrl(hFile, CtrlParam, STR, LINE)
  !***  PURPOSE:   to load the control parameter for events detection
  !
  !     INPUT:     hFile,  I/O unit number
  !                STR,    a string line
  ...
  implicit none
     ...
     !----
           ...
           do while(.TRUE.)
              ...
              !$$--- for none PARREP app, skip the section
              if(gm_AppType(1:len_trim(gm_AppType)) .ne. "PARREP") then
                 if(KEYWORD(1:LEN_TRIM(KEYWORD)) .eq. "&ENDSUBCTL") exit
                 cycle
              end if
              ...
              select case (KEYWORD(1:LEN_TRIM(KEYWORD)) )
                     ...
                     case ("&EXTRECORD", "&FINEREC", "&TAD_FINESTEP", "&PARREL_FINESTEP")
                          !$$--- To get for how many time steps to save current statu
                          call Extract_Numb(STR,2,n,STRNUMB)
                          if(N .LT. 1) then
                             write(*,fmt="(' MDPSCU Warning: parameter controling refining event checking is missed')")
                    ...
                    case ("&OUTPUT_C", "&EVENTCHECKSTEPS", "&TAD_CHECKSTEPS", "&PARREL_CHECKSTEPS")
                   ...
                   case ("&TERMINATEVENT", "&TAD_TERMINATEVENT", "&PARREP_TERMINATEVENT")
                         !$$--- To get if the time-section will exist at an event detected
                         call Extract_Numb(STR,1,n,STRNUMB)
                         if(N .LT. 1) then
                            write(*,fmt="(' MDPSCU Warning: value for controling parameter is missed')")
                   ...
                   case ("&TAD_DRTOL", "&PARREP_DRTOL", "&DRTOL")
                         !$$--- To get if the tolerance displacement for event detection
                         call Extract_Numb(STR,1,n,STRNUMB)
                         if(N .LT. 1) then
                            write(*,fmt="(' MDPSCU Warning: parameter controlling the structure change detetction is missed')")
                   ...
                   case ("&TAD_TLOW")
                         ...
                         call Extract_Numb(STR,1,n,STRNUMB)
                         if(N .LT. 1) then
                            write(*,fmt="(' MDPSCU Warning: low temperature input is missed')")
                            write(*,fmt="('                 check control file at line:', BZI6)") LINE
                            write(*,fmt="(' Usage:  &TAD_TLOW temperature at low')")
                            call ONWARNING(gm_OnWarning)
                         else
                            CtrlParam%TAD_TILow = DRSTR(STRNUMB(1))
                         end if

                   case ("&TAD_MUmin")
                         call Extract_Numb(STR,1,n,STRNUMB)
                         if(N .LT. 1) then
                            write(*,fmt="(' MDPSCU Warning: controlling parameter for TAD_MUmin is missed')")
                            write(*,fmt="('                 check control file at line:', BZI6)") LINE
                            write(*,fmt="(' Usage:  &TAD_MUmin temperature at low')")
                            call ONWARNING(gm_OnWarning)
                         else
                            CtrlParam%TAD_MUmin = DRSTR(STRNUMB(1))
                         end if
                   ...
                   case ("&TAD_DELAT")
                         ...
                         if(N .LT. 1) then
                            ...
                            CtrlParam%TAD_Delta = DRSTR(STRNUMB(1))
                         end if
  ...
  100    write(*,*)"MDPSCU Error in reading parallel-replica control parameters."
         ...
         write(*,*)"Please check if keyword &ENDSUBCTL exist to end the data subsection"
         stop
 end subroutine Load_TADCtl_SimMDCtrl
  !****************************************************************************
                ...
                select case (KEYWORD(1:LEN_TRIM(KEYWORD)) )
                     ...
                     case ("&COMMSUBCTL")
                       ...
                       case ("&SECTSUBCTL")
                             ...
                             ISECT = ISECT+1
                             call Load_SectionParameter_SimMDCtrl(hFile, curCtrlParam, SimBox, STR,  LINE, ISECT)
     ...
     !----
        ...
        !$$**** HEADER
          write(hFile,fmt="(' !************ SIMULATION CONTROL PARAMETERS *******************')")
              ...
              !*************************
              write(hFile,FMT="(' !    Number of points in potential and force table.: ', 20(BNI8,1x))") CtrlParam%NUMFTABR
              write(hFile,FMT="(' !    Number of points in embedment function table..: ', 20(BNI8,1x))") CtrlParam%NUMFTABE
              ...
              do IS=1, nsect
                 ...
                 if(sectCtrlParam%DAMPTIME1  .LE. 0  .and. &
                    ...
                    if(sectCtrlParam%DAMPTIME1 .GT. 0) then
                       write(hFile,FMT="(' !       QuickDamping to be performed in section....: ', BZI5)") IS
